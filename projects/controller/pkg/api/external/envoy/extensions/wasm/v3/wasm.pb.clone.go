...
func (m *VmConfig) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetCode()).(clone.Cloner); ok {
		target.Code = h.Clone().(*github_com_solo_io_gloo_projects_controller_pkg_api_external_envoy_config_core_v3.AsyncDataSource)
	} else {
		target.Code = proto.Clone(m.GetCode()).(*github_com_solo_io_gloo_projects_controller_pkg_api_external_envoy_config_core_v3.AsyncDataSource)
	}
