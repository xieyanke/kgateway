	...
	if m.GetAllowedVaryHeaders() != nil {
		...
		for idx, v := range m.GetAllowedVaryHeaders() {
			...
			if h, ok := interface{}(v).(clone.Cloner); ok {
				target.AllowedVaryHeaders[idx] = h.Clone().(*github_com_solo_io_gloo_projects_controller_pkg_api_external_envoy_type_matcher_v3.StringMatcher)
			} else {
				target.AllowedVaryHeaders[idx] = proto.Clone(v).(*github_com_solo_io_gloo_projects_controller_pkg_api_external_envoy_type_matcher_v3.StringMatcher)
			}
