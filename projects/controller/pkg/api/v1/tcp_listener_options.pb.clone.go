...
import (
	...
	github_com_solo_io_gloo_projects_controller_pkg_api_v1_options_tcp "github.com/solo-io/gloo/projects/controller/pkg/api/v1/options/tcp"
)
...
func (m *TcpListenerOptions) Clone() proto.Message {
	...
	if h, ok := interface{}(m.GetTcpProxySettings()).(clone.Cloner); ok {
		target.TcpProxySettings = h.Clone().(*github_com_solo_io_gloo_projects_controller_pkg_api_v1_options_tcp.TcpProxySettings)
	} else {
		target.TcpProxySettings = proto.Clone(m.GetTcpProxySettings()).(*github_com_solo_io_gloo_projects_controller_pkg_api_v1_options_tcp.TcpProxySettings)
	}
