...
func (s *testingSuite) TestOTelTracing() {
	...
	s.TestInstallation.Assertions.AssertEventualCurlResponse(
		...
		[]curl.Option{
			...
			curl.WithHeader("x-header-tag", "value"),
	...
	s.eventuallyFindRequestInCollectorLogs([]string{
		...
		`-> request: Str(value)`,
	}, "should find traces in collector pod logs")
}

// eventuallyFindRequestInCollectorLogs checks if the collector pod logs contain the expected patterns
func (s *testingSuite) eventuallyFindRequestInCollectorLogs(lines []string, msg string) {
	...
	}, time.Second*60, time.Second*15, msg).Should(gomega.Succeed())
}
